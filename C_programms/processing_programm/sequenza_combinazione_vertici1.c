...
/* Questa parte di programma serve a risolvere i vertici e le parit‡, se esistenti. Deve ancora essere finito. Andr‡ messo prima del programma "sequenza combinazione pezzi" */
...
int main(int argc, char *argv[]) {
	...
	int end=0;																						//variabile che mi far‡ uscire dal ciclo pi˘ grande
	int end_int=0;																					//variabile che mi far‡ uscire dal ciclo pi˘ interno
	int d=0;																						//contatore
	int i=0;																						//contatore
	int n=0;																						//variabile che conterr√  l'informazione del numero da mettere
	int m=0;																						//variabile che conterr√  l'informazione del numero del buffer
		//identifico vertici[0][0], [0][1] e [0][2] come zona di buffer, quando in realt‡  sar‡  la zona a;n;q
	...
	do{
		if (vertici_vero[0][0]==vertici[0][0] && vertici_vero[0][1]==vertici[0][1] && vertici_vero[0][2]==vertici[0][2]){	//se nel buffer ci sono i colori giusti e messi bene
			for (i=0;i<8;i++){
				for (d=0;d<3;d++){
					if (vertici_vero[i][d]!= vertici[i][d]){																//se nella posizione analizzata ci sono colori diversi -> cubo non risolto
						...
						vertici[0][0]=n;//metto in informazione al buffer il primo colore del luogo dove il colore Ë sbagliato
						...
						vertici[0][1]=n;//metto in informazione al buffer il secondo colore del luogo dove il colore Ë sbagliato
						...
						vertici[0][2]=n;//metto in informazione al buffer l'ultimo colore del luogo dove il colore Ë sbagliato
						...
						end_int=1;		//imposto la variabile a 1 per far si che quando usir√† dal primo for sar√† costretto ad andare nell'altro e chiudere anch'esso
						break;			//esco dal for
					}
					else if(i!=7 && vertici[i][d]==vertici_vero[i][d]){
						//non fa nulla se non √® l'ultimo confronto e se i valori confrontati sono uguali
					}
				...
				if (end_int==1){			//se la variabile per far chiudere tutti i for √® stata attivata
			...
			for (i=0;i<8;i++){
				for(d=0;d<3;d++){
					if (i==0){//se i Ë in zona buffer 
						//non fa nulla, perch√® senn√≤ scambierebbe in loop la posizione di buffer messa male
					}
					else if(vertici[i][d]!=vertici_vero[i][d]){//se nella posizione ci sono colori diversi-> cubo non risolto (devo controllare questo if perch√® non sono convinto di come funzioni)
						q
					}
				}
			}
	...
	do{
		if (array[0]==avero[0]){								//se il pezzo nel buffer Ë messo correttamente
			for(i=0;i<8;i++){
				if(array[i]==avero[i]){							//se la posizione Ë uguale a quella definitiva
					if(i==7){									//se ha controllato tutte le posizioni e sono ok
						end=1;									//imposto la variabile che mi far‡ uscire dal ciclo Ë pi˘ grande
						break;									//esco dal for
					}
				}
				else if(array[i]!=avero[i]){					//se la posizione non Ë uguale a quella definitiva
		...
		else if(array[0]!=avero[0]){							//se il pezzo nel buffer non Ë messo bene
			for(i=0;i<8;i++){									//controllo la posizione in cui dovrebbe essere messo
				if(array[0]==avero[i]){							//se l'informazione contenuta nel buffer Ë uguale a quella della posizione analizzata
					printf("%d\n", i);							//stampo il cambiamento che ho fatto
					d=array[i];									//salvo l'informazione del luogo da permutare in una variabile			
					array[i]=array[0];							//metto nel luogo da permutare il pezzo giusto
					array[0]=d;									//salvo nel buffer l'informazione del pezzo permutato
