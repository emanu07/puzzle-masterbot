...
int commutazioni_ver(int d, int i, int vertici_ins[8][3]){//parte di programma che identifica come permutare ogni singolo vertice in base alla posizione e che mi restituirÃƒÆ’Ã‚Â  l'informazione per risolvere i vertici uno ad uno
	int a=0;//variabile che conterrÃƒÆ’Ã‚Â  l'informazione da mettere
	int b=0;//variabile che conterrÃƒÆ’Ã‚Â  l'informazione del buffer
	if(d==0){//se la prima posizione scombinata che troviamo ÃƒÆ’Ã‚Â¨ quella relativa alla prima postazione codificata del vertice
		switch(i){
			case 0:{//se i ÃƒÆ’Ã‚Â¨ uguale a 0 (impossibile ma la mettiamo lo stesso, non puÃƒÆ’Ã‚Â² commutare la stessa zona di buffer), avvia la risoluzione per la posizione "a"
				printf("error\n");//stampa errore
				break;//esce dallo switch
			}
			case 1:{//se i ÃƒÆ’Ã‚Â¨ uguale a 1, avvia la risoluzione per la posizione "b"
			...
			case 2:{//se i ÃƒÆ’Ã‚Â¨ uguale a 2, avvia la risoluzione per la posizione "c"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 3:{//se i ÃƒÆ’Ã‚Â¨ uguale a 3, avvia la risoluzione per la posizione "d"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 4:{//se i ÃƒÆ’Ã‚Â¨ uguale a 4, avvia la risoluzione per la posizione "o"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 5:{//se i ÃƒÆ’Ã‚Â¨ uguale a 5, avvia la risoluzione per la posizione "k"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 6:{//se i ÃƒÆ’Ã‚Â¨ uguale a 6, avvia la risoluzione per la posizione "g"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 7:{//se i ÃƒÆ’Ã‚Â¨ uguale a 7, avvia la risoluzione per la posizione "h"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
	...
	else if  (d==1){//se la prima posizione scombinata che troviamo ÃƒÆ’Ã‚Â¨ quella relativa alla seconda postazione codificata del vertice
		switch(i){
			case 0:{//se i ÃƒÆ’Ã‚Â¨ uguale a 0 (impossibile ma la mettiamo lo stesso, non puÃƒÆ’Ã‚Â² commutare la stessa zona di buffer), avvia la risoluzione per la posizione "n"
				printf("error\n");//stampa errore
				break;//esce dallo switch
			}
			case 1:{//se i ÃƒÆ’Ã‚Â¨ uguale a 1, avvia la risoluzione per la posizione "m"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 2:{//se i ÃƒÆ’Ã‚Â¨ uguale a 2, avvia la risoluzione per la posizione "i"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 3:{//se i ÃƒÆ’Ã‚Â¨ uguale a 3, avvia la risoluzione per la posizione "e"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 4:{//se i ÃƒÆ’Ã‚Â¨ uguale a 4, avvia la risoluzione per la posizione "t"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 5:{//se i ÃƒÆ’Ã‚Â¨ uguale a 5, avvia la risoluzione per la posizione "p"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 6:{//se i ÃƒÆ’Ã‚Â¨ uguale a 6, avvia la risoluzione per la posizione "l"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
				...
				//cubo giÃƒÂ  in posizione
				perm();//stampa la permutazione dei vertici
				//cubo giÃƒÂ  rimesso a posto
				break;//esce dallo switch
			}
			case 7:{//se i ÃƒÆ’Ã‚Â¨ uguale a 7, avvia la risoluzione per la posizione "s"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
	...
	else if(d==2){//se la prima posizione scombinata che troviamo ÃƒÆ’Ã‚Â¨ quella relativa all'ultima postazione codificata del vertice
		switch(i){
			case 0:{//se i ÃƒÆ’Ã‚Â¨ uguale a 0 (impossibile ma la mettiamo lo stesso, non puÃƒÆ’Ã‚Â² commutare la stessa zona di buffer), avvia la risoluzione per la posizione "q"
				printf("error\n");//stampa errore
				break;//esce dallo switch
			}
			case 1:{//se i ÃƒÆ’Ã‚Â¨ uguale a 1, avvia la risoluzione per la posizione "j"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 2:{//se i ÃƒÆ’Ã‚Â¨ uguale a 2, avvia la risoluzione per la posizione "f"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 3:{//se i ÃƒÆ’Ã‚Â¨ uguale a 3, avvia la risoluzione per la posizione "r"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 4:{//se i ÃƒÆ’Ã‚Â¨ uguale a 4, avvia la risoluzione per la posizione "z"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 5:{//se i ÃƒÆ’Ã‚Â¨ uguale a 5, avvia la risoluzione per la posizione "x"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 6:{//se i ÃƒÆ’Ã‚Â¨ uguale a 6, avvia la risoluzione per la posizione "v"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
			...
			case 7:{//se i ÃƒÆ’Ã‚Â¨ uguale a 7, avvia la risoluzione per la posizione "u"
				a=vertici_ins[0][0];//associo ad a la prima informazione di buffer
...
int commutazioni(int d, int i){//funzione per togliere moltre stringhe di codice per capire quale ciclo sta facendo per risolvere il cubo
	if(d==0){//se la posizione scombinata ÃƒÂ¨ quella della prima informazione della cella
		switch(i){
			case 0:{//se i ÃƒÂ¨ uguale a 0, si avvia la risoluzione per la posizione a
				printf("3 move to sx\n action 180\n rotate 180\n move to sx\n action 90\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate to sx\n action 180\n 3 move to sx\n action 270\n 3 move to sx\n rotate 180\n action 180\n rotate to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 1:{//se i ÃƒÂ¨ uguale a 1 (impossibile ma la mettiamo lo stesso), avvia la risoluzione per la posizione b
				printf("error\n");//stampa errore
				break;//esce dallo switch
			}
			case 2:{//se i ÃƒÂ¨ uguale a 2,si avvia la risoluzione per la posizione c
				printf("3 move to sx\n action 180\n rotate 180\n move to sx\n action 270\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n action 90\n 3 move to sx\n rotate 180\n action 180\n move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 3:{//se i ÃƒÂ¨ uguale a 3, si avvia la risoluzione per la posizione d
				t_perm();//svolge la t perm
				break;//esce dallo switch
			}
			case 4:{//se i ÃƒÂ¨ uguale a 4, si avvia la risoluzione per la posizione f
				printf("2 move to sx\n action 180\n move to sx\n action 270\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 90\n 3 move to sx\n action 180\n 2 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 5:{//se i ÃƒÂ¨ uguale a 5, si avvia la risoluzione per la posizione g
				printf("3 move to sx\n action 270\n rotate 90\n move to sx\n action 270\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n action 90\n 3 move to sx\n rotate 270\n action 90\n move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 6:{//se i ÃƒÂ¨ uguale a 6, si avvia la risoluzione per la posizione h
				printf("move to sx\n action 90\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 270\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 7:{//se i ÃƒÂ¨ uguale a 7, si avvia la risoluzione per la posizione j
				printf("2 move to sx\naction 270\n rotate 90\n 3 move to sx\n action 270\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 90\n move to sx\n rotate 270\n action 90\n 2 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 8:{//se i ÃƒÂ¨ uguale a 8, si avvia la risoluzione per la posizione k
				printf("action 90\n rotate 90\n move to sx\n action 270\n rotate 90\n 3 move to sx\n action 90\n move to sx\n action 90\n rotate 90\n move to sx\n rotate 90\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 270\n 3 move to sx\n rotate 270\n action 270\n 3 move to sx\n action 270\n move to sx\n rotate 270\n action 90\n 3 move to sx\n rotate 270\n action 270\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 9:{//se i ÃƒÂ¨ uguale a 9, si avvia la risoluzione per la posizione n
				printf("move to sx\n action 270\n 3 move to sx\n\n ");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 90\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 10:{//se i ÃƒÂ¨ uguale a 10, si avvia la risoluzione per la posizione o
				printf("3 move to sx\n action 90\n rotate 90\n 3 move to sx\n action 90\n 3 move to sx\n action 180\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 180\n move to sx\n action 270\n move to sx\n rotate 270\n action 270\n move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 11:{//se i ÃƒÂ¨ uguale a 11, si avvia la risoluzione per la posizione s
				printf("move to sx\n action 90\n move to sx\n action 90\n rotate 90\n move to sx\n action 270\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 90\n 3 move to sx\n rotate 270\n action 270\n 3 move to sx\n action 270\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
	...
	else if(d==1){//se la posizione scombinata ÃƒÂ¨ quella dell'ultima informazione della cella
		switch(i){
			case 0:{//se i ÃƒÂ¨ uguale a 0, si avvia la risoluzione per la posizione m
				printf("3 move to sx\n action 90\n rotate 90\n 3 move to sx\n action 270\n 3 move to sx\n action 180\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 180\n move to sx\n action 90\n move to sx\n rotate 270\n action 270\n move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 1:{//se i ÃƒÂ¨ uguale a 1 (impossibile ma la mettiamo lo stesso), avvia la risoluzione per la posizione i
				printf("error\n");//stampa il cambiamento fatto per conoscere il ciclo della risoluzione del cubo
				break;//esce dallo switch
			}
			case 2:{//se i ÃƒÂ¨ uguale a 2, si avvia la risoluzione per la posizione e
				printf("3 move to sx\n action 270\n rotate 90\n move to sx\n action 90\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n action 270\n 3 move to sx\n rotate 270\n action 90\n move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 3:{//se i ÃƒÂ¨ uguale a 3, si avvia la risoluzione per la posizione q
				printf("move to sx\n action 270\n move to sx\n action 90\n rotate 90\n move to sx\n action 270\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 90\n 3 move to sx\n rotate 270\n action 270\n 3 move to sx\n action 90\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 4:{//se i ÃƒÂ¨ uguale a 4, si avvia la risoluzione per la posizione l
				printf("2 move to sx\n action 90\n rotate 90\n move to sx\n action 90\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 270\n 3 move to sx\n rotate 270\n action 270\n 2 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 5:{//se i ÃƒÂ¨ uguale a 5, si avvia la risoluzione per la soluzione u
				printf("action 90\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n action 270\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 6:{//se i ÃƒÂ¨ uguale a 6, si avvia la risoluzione per la soluzione r
				printf("2 move to sx\n action 90\n rotate 90\n move to sx\n action 270\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 90\n 3 move to sx\n rotate 270\n action 270\n 2 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 7:{//se i ÃƒÂ¨ uguale a 7, si avvia la risoluzione per la soluzione p
				printf("2 move to sx\n action 180\n move to sx\n action 90\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 270\n 3 move to sx\n action 180\n 2 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 8:{//se i ÃƒÂ¨ uguale a 8, si avvia la risoluzione per la soluzione v
				printf("action 180\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n action 180\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 9:{//se i ÃƒÂ¨ uguale a 9, si avvia la risoluzione per la soluzione t
				printf("2 move to sx\n action 270\n rotate 270\n move to sx\n action 90\n move to sx\n rotate 180\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("rotate 180\n 3 move to sx\n action 270\n 3 move to sx\n rotate 90\n action 90\n 2 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 10:{//se i ÃƒÂ¨ uguale a 10, si avvia la risoluzione per la soluzione x
				printf("action 270\n move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n action 90\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
			}
			case 11:{//se i ÃƒÂ¨ uguale a 11, si avvia la risoluzione per la soluzione z
				printf("move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo
				t_perm();//svolge la t perm
				printf("move to sx\n action 180\n 3 move to sx\n\n");//stampa i movimenti per portare l'obbiettivo a posto
				break;//esce dallo switch
...
/*per non utilizzare altri servi motori per lo scan arm bisognerÃ  implementare al programma non il movimento dello scan arm verso up ma basterÃ  
ruotare di 45 gradi il cassetto per allineare solamente cosÃ¬ il braccio ai vertici*/
...
int main(int argc, char *argv[]) {
	...
	int n=0;								//creo una variabile che utilizzerÃ² come contatore
	...
	int lati [1][24];						//array che conterrÃ  tutte le informazioni sui lati scombinati e adimensionali
	int vertici[1][24];						//array che conterrÃ  tutte le informazioni sui vertici scombinati e adimensonali
	...
	//vertici deve in realtÃ  chiamarsi vertici_ins per combaciare col programma precedente
	int end=0;																						//variabile che mi farÃ Â  uscire dal ciclo piÃ¹ grande
	int end_int=0;																					//variabile che mi farÃ Â  uscire dal ciclo piÃ¹ interno
	int d=0;																						//contatore
	int i=0;																						//contatore
	//int n=0;																						//variabile che conterrÃ Â   l'informazione del numero da mettere --> basta rinominarlo =0 all'inizio della parte di programma
	int m=0;																						//variabile che conterrÃ Â   l'informazione del numero del buffer
	int parita=0;																					//contatore per vedere quante volte permuta i vertici, in modo da capire se eseguire una paritÃƒÂ  o meno
	...
	//int n=0;//variabile che conterrÃƒÂ  l'informazione del numero da mettere --> basta rinominarlo =0 all'inizio della parte di programma
	//int m=0;//variabile che conterrÃƒÂ  l'informazione del numero del buffer --> basta rinominarlo =0 all'inizio della parte di programma
	...
	//lati deve in realtÃ  chiamarsi lati_ins per combaciare col programma precedente
	int lati[12][2]={{2,1}, {6,4}, {6,5}, {5,2}, {3,1}, {2,6}, {5,4}, {1,5}, {4,1}, {3,6}, {4,3}, {2,3}};//array scomposto bidimensionale
	
	scanf("%d",& c1);						//guardo che colore Ã¨ il centro in cima
	printf("\n");
	while(c1 != 1){							//se il colore non Ã¨ bianco
		printf("move to sx\n");				//sposta cubo verso sinistra
		scanf("%d",& c1);					//guardo che colore Ã¨ il nuovo centro in cima
		printf("\n");						
		n++;								//aggiorno di uno il contatore
		if(n==3){							//se il contatore Ã¨ a 3
			printf("rotate 90\n");			//ruota il cubo di 90Â° orari
			n=0;							//aggiorna il contatore a 0
		}
	}
	printf("centro bianco trovato\n");		//per capire se funge
	printf("move to sx\n");					//sposta cubo verso sinistra
	scanf("%d",& c2);						//guardo che colore Ã¨ il centro che andrÃ  di fronte a me
	printf("\n");
	printf("rotate 90\n");					//ruota il cubo di 90Â° orari
	while(c2!=2){
		printf("move to sx\n");				//sposta cubo verso sinistra
		scanf("%d",& c2);					//guardo che colore Ã¨ il nuovo centro che andrÃ  di fronte a me
		printf("\n");
	}
	printf("centro verde trovato\n");		//per capire se funge
	printf("rotate 90\n");					//ruota il cubo di 90Â° orari
	printf("move to sx\n");					//sposta cubo verso sinistra
	printf("rotate 270\n");				//ruota il cubo di 270Â° orari
	...
	for(nf=0; nf<6; nf++){					//crea funzione per ogni faccia del cubo
		...
		else{								//se non Ã¨ ne la quarta che la quinta faccia
			printf("move to sx\n");			//sposta cubo verso sinistra
		}	
	}
	printf("rotate 270\n");					//ruota il cubo di 270Â° orari
	printf("move to sx\n");					//sposta cubo verso sinistra
	t=0;									//faccio sÃ¬ che la variabile t ritorni a 0 perchÃ¨ dovrÃ² riusarla per l'altro array
	...
	vertici_ins[0][0]= vertici[0][0];//vertice di coordinate a,n,q che deve prendere le informazioni del 1Ã‚Â°, del 16Ã‚Â° e del 18Ã‚Â° dell'array vertici
	vertici_ins[0][1]= vertici[0][15];
	vertici_ins[0][2]= vertici[0][17];
	vertici_ins[1][0]= vertici[0][3];//vertice di coordinate b,m,j che deve prendere le informazioni del 4Ã‚Â°, del 19Ã‚Â° e del 5Ã‚Â° dell'array vertici
	vertici_ins[1][1]= vertici[0][18];
	vertici_ins[1][2]= vertici[0][4];
	vertici_ins[2][0]= vertici[0][2];//vertice di coordinate c,i,f che deve prendere le informazioni del 3Ã‚Â°, del 6Ã‚Â° e del 22Ã‚Â° dell'array vertici
	vertici_ins[2][1]= vertici[0][5];
	vertici_ins[2][2]= vertici[0][21];
	vertici_ins[3][0]= vertici[0][1];//vertice di coordinate d,e,r che deve prendere le informazioni del 2Ã‚Â°, del 23Ã‚Â° e del 15Ã‚Â° dell'array vertici
	vertici_ins[3][1]= vertici[0][22];
	vertici_ins[3][2]= vertici[0][14];
	vertici_ins[4][0]= vertici[0][16];//vertice di coordinate o,t,z che deve prendere le informazioni del 17Ã‚Â°, del 13Ã‚Â° e del 12Ã‚Â° dell'array vertici
	vertici_ins[4][1]= vertici[0][12];
	vertici_ins[4][2]= vertici[0][11];
	vertici_ins[5][0]= vertici[0][7];//vertice di coordinate k,p,x che deve prendere le informazioni del 8Ã‚Â°, del 20Ã‚Â° e del 9Ã‚Â° dell'array vertici
	vertici_ins[5][1]= vertici[0][19];
	vertici_ins[5][2]= vertici[0][8];
	vertici_ins[6][0]= vertici[0][20];//vertice di coordinate g,l,v che deve prendere le informazioni del 21Ã‚Â°, del 7Ã‚Â° e del 10Ã‚Â° dell'array vertici
	vertici_ins[6][1]= vertici[0][6];
	vertici_ins[6][2]= vertici[0][9];
	vertici_ins[7][0]= vertici[0][23];//vertice di coordinate h,s,u che deve prendere le informazioni del 24Ã‚Â°, del 14Ã‚Â° e del 11Ã‚Â° dell'array vertici
	...
	lati_ins[0][0]= lati[0][3];//lato di coordinate a,m che deve prendere le informazioni del 4Â° e del 18Â° dell'array lati
	lati_ins[0][1]= lati[0][17];
	lati_ins[1][0]= lati[0][2];//lato di coordinate b,i (buffer) che deve prendere le informazioni del 3Â° e del 5Â° dell'array lati
	lati_ins[1][1]= lati[0][4];
	lati_ins[2][0]= lati[0][1];//lato di coordinate c,e che deve prendere le informazioni del 2Â° e del 22Â° dell'array lati
	lati_ins[2][1]= lati[0][21];
	lati_ins[3][0]= lati[0][0];//lato di coordinate d,q che deve prendere le informazioni del 1Â° e del 15Â° dell'array lati
	lati_ins[3][1]= lati[0][14];
	lati_ins[4][0]= lati[0][20];//lato di coordinate f,l che deve prendere le informazioni del 21Â° e del 6Â° dell'array lati
	lati_ins[4][1]= lati[0][5];
	lati_ins[5][0]= lati[0][23];//lato di coordinate g,u che deve prendere le informazioni del 24Â° e del 10Â° dell'array lati
	lati_ins[5][1]= lati[0][9];
	lati_ins[6][0]= lati[0][22];//lato di coordinate h,r che deve prendere le informazioni del 23Â° e del 14Â° dell'array lati
	lati_ins[6][1]= lati[0][13];
	lati_ins[7][0]= lati[0][7];//lato di coordinate j,p che deve prendere le informazioni del 8Â° e del 19Â° dell'array lati
	lati_ins[7][1]= lati[0][18];
	lati_ins[8][0]= lati[0][6];//lato di coordinate k,v che deve prendere le informazioni del 7Â° e del 9Â° dell'array lati
	lati_ins[8][1]= lati[0][8];
	lati_ins[9][0]= lati[0][16];//lato di coordinate n,t che deve prendere le informazioni del 17Â° e del 16Â° dell'array lati
	lati_ins[9][1]= lati[0][15];
	lati_ins[10][0]= lati[0][19];//lato di coordinate o,x che deve prendere le informazioni del 20Â° e del 12Â° dell'array lati
	lati_ins[10][1]= lati[0][11];
	lati_ins[11][0]= lati[0][12];//lato di coordinate s,z che deve prendere le informazioni del 13Â° e del 11Â° dell'array lati
	lati_ins[11][1]= lati[0][10];
	
	
	/*Questa parte di programma serve a risolvere i vertici e ad effettuare le paritÃ */
	
	
	//identifico vertici[0][0], [0][1] e [0][2] come zona di buffer, quando in realtÃƒÂ Ã‚Â   sarÃƒÂ Ã‚Â   la zona a;n;q
	n=0;//rimetto n=0 perchÃ¨ Ã¨ stata usata da numerosi programmi	
	do{
		...
		if (vertici_vero[0][0]==vertici_ins[0][0] && vertici_vero[0][1]==vertici_ins[0][1] && vertici_vero[0][2]==vertici_ins[0][2]){	//se nel buffer ci sono i colori giusti e messi bene
			for (i=0;i<8;i++){
				for (d=0;d<3;d++){
					if (vertici_vero[i][d]!= vertici_ins[i][d]){	//se nella posizione analizzata ci sono colori diversi -> cubo non risolto (molto probabilmente bisogna mettere anche qui il pezzo di codice di commutazione come nel cubo risolto ma messo male
						...
						parita=parita+1;//aggiorno il contatore delle paritÃƒÂ  
						end_int=1;		//imposto la variabile a 1 per far si che quando uscirÃƒÂ  dal primo for sarÃƒÂ  costretto ad andare nell'altro e chiudere anch'esso
						break;			//esco dal for
					}
					else if(i!=7 && vertici_ins[i][d]==vertici_vero[i][d]){
						//non fa nulla se non ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ l'ultimo confronto e se i valori confrontati sono uguali
					}
					else if (i==7 && d==2) {//se tutti sono giusti 
						end=1;				//modifico la variabile end
						end_int=1;			//imposto la variabile a 1 per far si che quando usirÃƒÂ Ã‚Â  dal primo for sarÃƒÂ  costretto ad andare nell'altro e chiudere anch'esso
			...
			for (i=0;i<8;i++){
				for(d=0;d<3;d++){
					if (i==0){//se i ÃƒÂ¨ in zona buffer 
						//non fa nulla, perchÃƒÂ¨ sennÃƒÂ² scambierebbe in loop la posizione di buffer messa male
					}
					else if(vertici_ins[i][d]!=vertici_vero[i][d]){//se nella posizione ci sono colori diversi-> cubo non risolto (devo controllare questo if perchÃƒÂ¨ non sono convinto di come funzioni)
						commutazioni_ver(d, i, vertici_ins);//avvia la funzione commutazioni_ver
						parita=parita+1;//aggiorno il contatore delle paritÃƒÂ 
						end_int=1;		//imposto la variabile a 1 per far si che quando uscirÃƒÂ  dal primo for sarÃƒÂ  costretto ad andare nell'altro e chiudere anch'esso
						break;			//esco dal for							
					}
					else if(vertici_ins[i][d]==vertici_vero[i][d]){//se nella posizione analizzata il pezzo ÃƒÂ¨ giusto
			...
			for (i=0;i<8;i++){
				for(d=0;d<3;d++){
					...
					else if(vertici_ins[i][d]==vertici_vero[i][d]){//se nella posizione analizzata il pezzo ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ giusto
					//non faccio nulla e continuo con l'analisi
					} 
				}
				if (end_int==1){			//se la variabile per far chiudere tutti i for ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ stata attivata
					end_int=0;				//ristabilizzo la variabile per poterla riutilizzare
					break;					//faccio smettere anche questo for
				}
			}
		...
		else if (end!=1){//se il cubo non ÃƒÂ¨ risolto --> nel buffer ci sono colori completamente sbagliati
			for(i=0;i<8;i++){//confronta ogni vertice
				if(vertici_ins[0][0]==vertici_vero[i][0] || vertici_ins[0][0]==vertici_vero[i][1] || vertici_ins[0][0]==vertici_vero[i][2] ){//se nel vertice analizzato c'ÃƒÆ’Ã‚Â¨ anche solo un colore che combacia con quello della prima posizione del buffer
					if(vertici_ins[0][1]==vertici_vero[i][0] || vertici_ins[0][1]==vertici_vero[i][1] || vertici_ins[0][1]==vertici_vero[i][2]){//se nel vertice analizzato c'ÃƒÆ’Ã‚Â¨ anche solo un colore che combacia con quello dell seconda posizione del buffer
						if(vertici_ins[0][2]==vertici_vero[i][0] || vertici_ins[0][2]==vertici_vero[i][1] || vertici_ins[0][2]==vertici_vero[i][2]){//se nel vertice analizzato c'ÃƒÆ’Ã‚Â¨ anche solo un colore che combacia con quello dell'ultima posizione del buffer --> trovato vertice dove permutare il pezzo
	...
	else if(parita%2==1){//se sono state eseguite un numero dispari di commutazioni --> svolge l'algoritmo di paritÃƒÂ 
		printf("3 move to sx\n action 270\n 3 move to sx\n action 270\n move to sx\n action 90\n rotate 90\n move to sx\n action 90\n");//stampa la funzione
		printf("3 move to sx\n action 270\n rotate 90\n move to sx\n action 180\n 3 move to sx\n action 90\n move to sx\n action 180\n");//stampa la funzione
		printf("3 move to sx\n action 90\n rotate 90\n 3 move to sx\n action 270\n move to sx\n action 270\n rotate 90\n 3 move to sx\n");//stampa la funzione
		printf("action 270\n move to sx\n action 270\n 3 move to sx\n action 180\n move to sx\n action 90\n 3 move to sx\n action 90\n 2 move to sx\n");//stampa la funzione
	...
	//identifico lati [1][0] e [1][1] come zona di buffer, quando in realtÃƒÂ  sarÃƒÂ  la zona b;i
	d=0;//rimetto d=0 perchÃ¨ Ã¨ stata usata da numerosi programmi
	i=0;//rimetto i=0 perchÃ¨ Ã¨ stata usata da numerosi programmi
	n=0;//rimetto n=0 perchÃ¨ Ã¨ stata usata da numerosi programmi
	m=0;//rimetto m=0 perchÃ¨ Ã¨ stata usata da numerosi programmi
	end=0;//rimetto end=0 perchÃ¨ Ã¨ stata usata da numerosi programmi
	end_int=0;//rimetto end_int=0 perchÃ¨ Ã¨ stata usata da numerosi programmi
	do{
		if(lati[1][0]==lati_vero[1][0] && lati[1][1]==lati_vero[1][1]){//se nel buffer c'ÃƒÂ¨ il suo colore giusto e messo bene
			for(i=0;i<12;i++){
				for (d=0; d<2; d++){
					if(lati[i][d]!=lati_vero[i][d]){//se nella posizione ci sono colori diversi-> cubo non risolto
						...
						lati[1][0]=n;//metto in informazione al buffer il primo colore del luogo dove il colore ÃƒÂ¨ sbagliato
						...
						lati[1][1]=n;//metto in informazione al buffer l'ultimo colore del luogo dove il colore ÃƒÂ¨ sbagliato
			...
			for(i=0;i<12;i++){
				for (d=0; d<2; d++){
					if (i==1){//se i ÃƒÂ¨ in zona buffer 
						//non fa nulla, perchÃƒÂ¨ sennÃƒÂ² scambierebbe in loop la posizione di buffer messa male
					}
					else if(lati[i][d]!=lati_vero[i][d]){//se nella posizione ci sono colori diversi-> cubo non risolto (devo controllare questo if perchÃƒÂ¨ non sono convinto di come funzioni)
						...
						n=lati[i][0];//associo ad n l'informazione del luogo dove andrÃƒÂ  messa l'informazione del buffer
						...
						n=lati[i][1];//associo ad n l'informazione del luogo dove andrÃƒÂ  messa l'informazione del buffer
			...
			for(i=0;i<12;i++){
				//bisognerÃƒÂ  fare di nuovo degli if sia per il caso che siano nel modo corretto per la posizione o che siano messi al rovescio
